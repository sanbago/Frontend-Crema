...
import { DoctorObjType, ContactType } from '@crema/models/apps/Contact';
import {
  useContactActionsContext,
  useContactContext,
} from '../../context/ContactContextProvider';
import CreateDoctor from '../CreateContact';
...
const ContactListing = () => {
  ...
  const [selectedContact, setSelectedContact] = useState<DoctorObjType | null>(
    null
  );
  ...
  const onViewContactDetail = (contact: DoctorObjType) => {
    setSelectedContact(contact);
    onShowDetail(true);
  };

  const onOpenEditContact = (contact: DoctorObjType) => {
    setSelectedContact(contact);
  ...
  const onChangeStarred = (status: boolean, contact: DoctorObjType) => {
    const selectedIdList = [contact.userId];
    putDataApi<DoctorObjType[]>(
  ...
  const onUpdateSelectedContact = (contact: DoctorObjType) => {
    setContactData({
  ...
  const onUpdateContacts = (contacts: DoctorObjType[]) => {
    setContactData({
      data: contactList?.data.map((item) => {
  ...
  const onUpdateContact = (contact: DoctorObjType) => {
    setSelectedContact(contact);
